<?php
/**
 * Template Name: Special Deals
 * The template for displaying discounted special deals with teddy banner header.
...
$deal_toys = array_filter( $all_toys, function( $t ) {
    return ! empty( $t['is_deal'] );
} );
...
<div class="container" style="padding: 24px 20px 80px;">
    ...
    <div style="position: relative; border-radius: var(--radius-xl); overflow: hidden; min-height: 220px; display: flex; align-items: center; margin-bottom: 32px; box-shadow: var(--shadow-card);">
        ...
        <div style="position: relative; z-index: 2; padding: 36px 44px; max-width: 440px;">
            <span style="background: #EF4444; color: white; padding: 4px 12px; border-radius: var(--radius-full); font-size: 0.78rem; font-weight: 800; text-transform: uppercase;">
                Mega Savings Deals
            </span>
            <h1 style="font-size: 2.2rem; font-weight: 900; color: #0F172A; margin: 8px 0 6px;">Special Deals</h1>
            <p style="color: #334155; font-size: 0.95rem; line-height: 1.4;">
                Limited-time discounts on our soft plushies, educational toys, and STEM bundles!
            </p>
        </div>
    </div>
    ...
    <div class="shop-layout">
        ...
        <aside class="shop-sidebar">
            <div class="filter-header">
                <span style="font-family: var(--font-heading); font-weight: 800; font-size: 0.95rem;">Filter Deals</span>
                <a href="<?php echo esc_url( home_url( '/deals' ) ); ?>" style="font-size: 0.8rem; color: var(--accent-red); font-weight: 700;">Reset</a>
            </div>

            <div class="filter-group">
                <div class="filter-group-title">Categories</div>
                <?php
                $filter_categories = array( 'Soft Toys', 'Vehicles', 'Educational', 'Action Figures', 'Building Blocks' );
            ...
            <div class="filter-group">
                <div class="filter-group-title">Discount Amount</div>
                <label class="filter-option-item">
                    <input type="checkbox" />
                    <span>20% - 30% Off</span>
                </label>
        ...
        <div>
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 20px;">
                <?php foreach ( $deal_toys as $index => $toy ) : 
                    $btn_color = $button_colors[ $index % count( $button_colors ) ];
                ?>
                    <div class="demo-product-card">
                        <span class="demo-sale-tag" style="background:#EF4444;"><?php echo esc_html( $toy['badge'] ?: 'DEAL' ); ?></span>
                        ...
                        <div class="demo-card-actions">
                            ...
                            <a 
                                href="https://wa.me/923098444501?text=<?php echo urlencode( 'Hello! I want to order Special Deal: ' . $toy['name'] . ' (PKR ' . number_format( $toy['price'] ) . ')' ); ?>" 
                                target="_blank" 
                                rel="noreferrer"
