...
export default function SearchResults() {
  ...
  useEffect(() => {
    if (query) {
      setLoading(true);
      productService.search(query).then((results) => {
        setProducts(results);
        setLoading(false);
      });
    } else {
    ...
    <div className="container archive post-type-archive post-type-archive-product woocommerce-page" style={{ padding: '32px 20px 80px' }}>
      ...
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem' }}>
          {filteredProducts.length} matching {filteredProducts.length === 1 ? 'toy' : 'toys'} found.
        </p>
      </div>
        ...
        <main className="site-main">
          ...
          {sortedProducts.length > 0 ? (
            ...
            <div className="woocommerce-info" style={{ textAlign: 'center', padding: '60px 20px', background: 'white', borderRadius: 'var(--radius-xl)', border: '1px solid var(--gray-2)' }}>
              ...
              <h3 style={{ fontSize: '1.2rem', marginBottom: '6px' }}>No products found for "{query}".</h3>
              <p style={{ color: 'var(--text-muted)', marginBottom: '20px', fontSize: '0.9rem' }}>Try searching for another keyword or clear your filters.</p>
